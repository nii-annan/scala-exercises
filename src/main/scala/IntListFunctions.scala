...
object IntListFunctions {
  ...
  /**
    ...
    */
  def removeDuplicates(ints: List[Int]): List[Int] = ints.distinct
  ...
  /**
    ...
    */
  def isPalindrome(ints: List[Int]): Boolean = ints.reverse.equals(ints)
  ...
  def removeConsecutiveDuplicates(ints: List[Int]): List[Int] = {
    ints.tail.foldLeft(List(ints.head)) {
      (acc, int) =>
        if (acc.last == int) acc
        else acc :+ int
    }
